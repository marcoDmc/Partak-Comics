...
export default function ContentComponent({ children, name }: ContentProps) {
    return (<>
        <div className="border-[.1rem] h-full min-h-[350px]
         ...
         rounded-2xl p-1 font-normal cursor-pointer hover:scale-105
            transition-all hover:border-[4px] hover:border-rose">
            <span className="text-base pl-2 capitalize   text-beige italic
             items-center justify-between w-full col-start-1 col-span-1 row-start-1">
                <p>{name}</p>
            </span>
            {children}
            <div className="w-full text-beige text-base
            p-[0px] flex items-center justify-between gap-10">
                ...
                <div className="w-full p-[8px] text-xs uppercase max-w-[80px] text-center rounded-full border-[1px] h-full">pc / 01</div>
            </div>
        </div>
    </>)
}